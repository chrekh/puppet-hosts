...
class hosts (
  ...
  if $one_primary_ipv4 {
    $loopback_ipv4 = [ $lo_ipv4[0] ].filter |$elt| { $elt != undef and $elt != '' }
    $pri_ipv4 = [ $primary_ipv4[0] ].filter |$elt| { $elt != undef and $elt != '' }
  }
  else {
  ...
  if $one_primary_ipv6 {
    $loopback_ipv6 = [ $lo_ipv6[0] ].filter |$elt| { $elt != undef and $elt != '' }
    $pri_ipv6 = [ $primary_ipv6[0] ].filter |$elt| { $elt != undef and $elt != '' }
  }
  else {
